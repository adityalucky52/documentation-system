 ...
 * This guarantees no changes are lost when the staging draft is merged into live.
 */
export default function MergeConfirmModal({
  ...
  /**
   * Action: Saves latest draft edits and commits the merge request.
   */
  const confirmMergeEditsAction = async () => {
    if (!selectedPage || !user || !spaceId) return
    setIsMerging(true)
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/site/spaces/${spaceId}/merge`, {
        method: "PUT",
        headers: {
      ...
      <div className="w-full max-w-md bg-[#161618] border border-[#222225] rounded-xl p-6 shadow-2xl flex flex-col gap-4 animate-in zoom-in-95 duration-200 font-sans">
        ...
        <div className="flex items-center gap-3 text-white">
          ...
          <div className="flex flex-col">
            <h3 className="text-sm font-bold text-white">Merge edits to live</h3>
          </div>
        </div>
        ...
        {mergeSuccess ? (
          ...
          <div className="flex flex-col gap-4 py-2">
            <p className="text-xs text-[#8e8e93] leading-relaxed">
              Are you sure you want to merge these edits? This will write all edits directly to the live site.
            </p>
